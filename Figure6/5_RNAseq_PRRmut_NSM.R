...
samples<-read.table("source/samples.txt", head=TRUE);
...
files<-file.path("source/quants_salmon_NCBIref", 
                  paste(samples$Seq_ID, "quant", sep = "_"), "quant.sf");
length(files)
names(files)<-samples$Seq_ID;
file.exists(files);

tx2gene <- read.csv("source/NIP.tu2genes.csv", head=F);
txi <- tximport(files, type="salmon", tx2gene=tx2gene);
# the txi contains abundance, counts, length

# refRNA annotation was download from NCBI entrez FTP site, oryza sative
Os_entrez <- read.table("source/Oryza_sativa.gene_info", head=T, sep="\t",quote="");
Os_rapdb<- read.table("source/IRGSP-1.0_representative_annotation_2024-01-11.tsv", head=T, sep="\t",quote="",fill = TRUE)
Os_kegg<- read.table("source/kegg_list_osa.txt",head=T, sep="\t",quote="");
Os_funrice<-read.table("source/funrice_geneInfo.table.txt", header = T, sep="\t")
...
pdf(file.path(OUT_DIR,"Figure/Fig5_Sup_corrplot.pdf"))
...
p<-glimmaMDS(y,
             ...
             top=30000)
htmlwidgets::saveWidget(p, file.path(OUT_DIR, "Table/Fig5_RNA_seq_mdsplot.html"))
...
write.csv(degnumlist,paste0(OUT_DIR,"Table/Fig5b_edgeR_contrasts_degnumstats.csv"))
...
htmlwidgets::saveWidget(p, file.path(OUT_DIR, "Table/Fig5_1_Kit.MvsG.html"))
...
htmlwidgets::saveWidget(p, file.path(OUT_DIR, "Table/Fig5_2_fls2.MvsG.html"))
...
htmlwidgets::saveWidget(p, file.path(OUT_DIR, "Table/Fig5_3_cerk.MvsG.html"))
...
htmlwidgets::saveWidget(p, file.path(OUT_DIR, "Table/Fig5_4_fls2vsKit_germfree.html"))
...
htmlwidgets::saveWidget(p, file.path(OUT_DIR, "Table/Fig5_5_cerkvsKit_germfree.html"))
...
htmlwidgets::saveWidget(p, file.path(OUT_DIR, "Table/Fig5_6_fls2vsKit_microbiota.html"))
...
htmlwidgets::saveWidget(p, file.path(OUT_DIR, "Table/Fig5_7_cerkvsKit_microbiota.html"))
...
htmlwidgets::saveWidget(p, file.path(OUT_DIR, "Table/Fig5_8_con_fls_x_microbiota.html"))
...
htmlwidgets::saveWidget(p, file.path(OUT_DIR, "Table/Fig5_9_con_cerk_x_microbiota.html"))
...
p<-plot_grid(p1,p2)
p
ggsave(file.path(OUT_DIR,"Figure/Fig5_Supvenn_osfls2.pdf"),p)

UpSetR::fromList(fls2_DEG_ID)
pdf(file.path(OUT_DIR, "Figure/Fig5_Supfls2_UpSet_draw.pdf"))
...
pdf(file=file.path(OUT_DIR,"Figure/Fig5c_Sup_fls2_cpm_heatmap.pdf"),width=4,height = 10)
p
dev.off()
...
pdf(paste0(OUT_DIR,"Figure/Fig5c.ht_fls2_degs.pdf"))
ht1+ht2
dev.off()        
...
fls2_bingo_res<- readxl::read_excel("source/fls2_vs_Kit_integrated_GO.xlsx")
...
print(combined_plot)
ggsave("output/Figure/Fig5d.BiNGO_point_GOn_thred3.pdf", width=7, height=9)
...
pdf(file=file.path(OUT_DIR,"Figure/Fig5c_Sup_cerk1_cpm_heatmap.pdf"),width=4,height = 10)
p
dev.off()
## Combined the logFC and pvalue together
## Check the row names in different datasets
...
pdf(paste0(OUT_DIR,"Figure/Fig5c.ht_cerk1_degs.pdf"))
ht1+ht2
dev.off()        
...
write.csv(cerk1_deg_for_go, "output/Table/Table.cerk1_deg_for_go.csv")
...
cerk1_bingo_res<- readxl::read_excel("source/cerk1_group_BiNGO.xlsx")
...
ggsave("output/Figure//Fig5d.BiNGO_cerk1_GO_reponse.pdf", width=7, height=4)
...
ggsave("output/Figure/Fig5d.BiNGO_cerk1_GO_all.pdf", width=13, height=9)

library(cowplot)
combined_plot <- plot_grid(p1 , p2 ,ncol = 1, align = "v", rel_heights  = c(5, 6))
print(combined_plot)
ggsave("output/Figure/Fig5d.BiNGO_cerk1_GOn.pdf", width=13, height=9)
