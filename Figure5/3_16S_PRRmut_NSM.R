...
path<- "~/Documents/Research/LabMember/xjw/Publish/iMeta_github/Xu_et_al_2024-main/script/update/Figure5/"
setwd(path)
load("source/3_16S_PRRmut_NSM_dada2.RData")
...
metadata<-read.csv("source/metadata.csv",header = T,row.names = 1)
...
write.csv(data,"output/Table/Fig4d.Sup_Table1.rarefy_33742_tab.csv")
write.csv(tax_table(ps.removeinput),"output/Table/Fig4d.Sup_Table1.rarefy_33742_tax.csv")

taxonomy<- read.csv("output/Table/Fig4d.Sup_Table1.rarefy_33742_tax.csv",header = T,row.names = 1,sep=",")
...
boxdata<- read.csv("source/Table3.lefse_all_difftaxa_boxplot.csv",header = T)
boxdata<- boxdata %>%
...
ggsave("output/Figure/Fig4g_h.Figure.lefse_box_keep.pdf",c,width=4,height = 10)
...
write.csv(stats,"output/Table/Fig4d_Sup_NGS_stats_metadata_alpha.csv")
