...
setwd("~/Documents/Research/LabMember/xjw/Publish/iMeta_github/Xu_et_al_2024-main/script/update/Figure2/")
load("source/dada2.RData") # download site: https://figshare.com/ndownloader/files/59183102
# 
...
asv.tax<- read.csv("source/Table0.asvtax.csv",header = T,row.names = 1)
asv.tab<- read.csv("source/Table0.asvtab.csv",header = T,row.names = 1)
metadata<- read.table("source/1.sample-metadata.csv",sep=",",header = T)
...
source("../function/ps_filter.R")

sample_data(two_WT_ps) %>% data.frame() %>% dplyr::count(Batch, Compartment, Geno)

# Basic control: remove taxa occurred less than 6 samples.
ps.filter<- ps_filter(two_WT_ps, prev_n = 6, outdir = "output/Table")
ps.filt<- ps.filter$ps.filter #1104 taxa and 36 samples 
write.csv(tax_table(ps.filt), "output/Table/Table0.asvtax_WT_associated.csv")
write.csv(otu_table(ps.filt), "output/Table/Table0.asvtab_WT_associated.csv")
...
ggsave("output/Figure/Figure1.filter.pdf", width=6, height = 8)


### Diversity
source("../function/alpha_div.R")
sampledata<- data.frame(sample_data(ps.filt))
...
## Figure2A-B
alpha_div(ps = ps.filt, metadata= sampledata, outdir="output/",
          group_levels = group_levels,
          colorRGB = colorRGB, xlab_title = "Group",  tabidx = 1, figidx = 2)

## Figure2C
source("../function/beta_div.R")
beta_div(ps.filt, sample_data_ps=sampledata, colname="sampleid", group_levels= group_levels,
         colorRGB=colorRGB, outdir="output/", tabidx=1, figidx=2, perm_group = c("Compartment","Geno","Batch"))
...
retain_prevalence <- function(ps, tabind = 1, figind = 1, filtN = 0, RA = 0, prevN = 0, 
  ...
  process_ps <- function(subps, varname) {
    ...
    RA_stats <- prev.ra %>%
      ...
      left_join(prev.ra %>% dplyr::count(condition), by = "condition") %>%
...
sample_data(ps.filt) %>% data.frame() %>% dplyr::count(Batch, Compartment, Geno)
...
list_1<- retain_prevalence(ps_list_1, filtN = 0,  RA = 0.01, prevN = 5/6, 
                           out_dir = "output/", tabind = 1, figind = 1,grid_ncol = 4,
                           grid_width = 18, grid_height = 12,var_name = "ra0.01")

list_1$RA_stats_df_total %>%
  write.csv("output/Table/Table_conditionsummary1_res_RA0.01_prev0.83.csv")

list_1$process_df_total%>%
  write.csv("output//Table/Table_conditionsummary2_res_RA0.01_prev0.83.csv")
...
res_mh_rhizo<- read.csv("output/Table/Table1.ps_mh_rhizo_prev0.833333333333333_RA0.01_filtN0.csv", header = T, row.names = 1)
res_mh_root<- read.csv("output/Table/Table1.ps_mh_root_prev0.833333333333333_RA0.01_filtN0.csv", header = T, row.names = 1)
res_kit_rhizo<- read.csv("output/Table/Table1.ps_kit_rhizo_prev0.833333333333333_RA0.01_filtN0.csv", header = T, row.names = 1)
res_kit_root<- read.csv("output/Table/Table1.ps_kit_root_prev0.833333333333333_RA0.01_filtN0.csv", header = T, row.names = 1)
...
write.csv(all_res, "output/Table/Table_all_res_RA0.01_prev0.83.csv")
all_res %>%
  ...
  write.csv("output/Table/Table_filt_res_RA0.01_prev0.83.csv")
...
ggsave("output/Figure/Figure2E.Venn.pdf", width=5, height=5)
...
venn2<- ggvenn::ggvenn(group_present_id,
                       fill_color = c(colorRGB[c(2,3,5,6)]))
ggsave("output/Figure/Figure2D.Venn2.pdf", width=5, height=5)


#### PartV. pNST (Figure 2F) ####
asv_seq<- Biostrings::readDNAStringSet("source/asv_seq_20250207.fa")
...
group<- group %>% 
  ...
  select(treatment)
phylo<-read.tree("source//tree.nwk")
...
head(betaMNTD)
# write.csv(betaMNTD, 'output/Table/betaMNTD.csv', quote = FALSE, row.names= FALSE)

betaMNTD<- read.csv("source/betaMNTD.csv")
...
ggsave("output/Figure/Figure1.AssemblyEvaluation.pdf",p, width=15, height=5)
