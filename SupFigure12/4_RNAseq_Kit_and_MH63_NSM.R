...
# Differential analysis using DESeq2
  # Fig3a-b. Volcano plot  ~ SupFigure12
  # Fig3c. heatmap
  # Fig3d. GO enrichment visualization ~ SupFigure12
  # Fig3e. KEGG enrichment visualization ~ SupFigure12

setwd("update/SupFigure12/")
library(tximport)
library(pheatmap)
source("../function/save_pheatmap.R")

## 1. Load TPM results from Salmon
# path<- "~/Documents/Research/LabMember/xjw/Publish/DataAvailibility/Code/"
# setwd(path)

tx2gene<- read.csv("./source/tx2gene.csv",header = T)
dim(tx2gene)
head(tx2gene)
path<- "source/"
quant.sf<- file.path("source","quant",list.files("source/quant"))
names(quant.sf)<- gsub(".quant.sf","",list.files("source/quant"))
...
save_pheatmap_pdf(sampabd_pdf,"output/Figure/Fig3_Sup_sample_abundance_cor.pdf")
sampct_pdf<- pheatmap(sampct_cor, display_numbers = T,fontsize = 10, angle_col = 45)
save_pheatmap_pdf(sampct_pdf,"output/Figure/Fig3_Sup_sample_counts_cor.pdf")
...
library(DESeq2)
# Design
design<- read.csv("./source/Table2.Design.csv",header = T,row.names = 1)
...
subquant.sf<- quant.sf[grepl("MH63",basename(file.path(path,"quant",list.files("source/quant"))))]
...
write.csv(res,"output/Table/Fig3_Sup_MH63_res_all.csv")
...
write.csv(res_total,"output/Table/Fig3_Sup_MH63_sig_all.csv")
...
## Kit
#Tximport
subquant.sf<- quant.sf[grepl("kit",basename(file.path(path,"quant",list.files("source/quant"))))]
subtxi <- tximport(subquant.sf, type="salmon", 
                   tx2gene=tx2gene[,c("Name", "GeneID")], 
                   countsFromAbundance="lengthScaledTPM")
...
res <- res[order(res$pvalue, res$log2FoldChange, decreasing = c(FALSE, TRUE)), ]
res <- cbind(subtxi$abundance[rownames(res),],res)
write.csv(res,"output/Table/Fig3_Sup_Kit_res_all.csv")
...
write.csv(res_total,"output/Table/Fig3_Sup_Kit_sig_all.csv")
...
Kit_res<- read.csv("output/Table/Fig3_Sup_Kit_res_all.csv",header = T,row.names = 1)
Kit_res
...
MH_res<- read.csv("output/Table/Fig3_Sup_MH63_res_all.csv",header = T,row.names = 1)
MH_res
...
ggsave("output/Figure/Fig4b.MH63_volcano.pdf",width = 12.5,height = 10,units = "cm")
...
gobp<- read.delim("source/Table2.GO_BP_sig0.01.txt",sep="\t",header = T)
...
ggsave("output/Figure/Fig3d.GO_col2.pdf",height = 3,width = 7.5)


## KEGG
kegg_all<- read.csv("source/KEGG_merge.txt",header = T,sep="\t")
...
ggsave("output/Figure/Fig3e.KEGG.pdf",width = 8,height = 5)
write.csv(kegg_all,"output/Table/Fig3e.KEGG_plot.csv")
...
par(mfrow = c(2,1))
pdf("output/Figure/Fig3_Sup_normalize.pdf",width=10,height = 5)
